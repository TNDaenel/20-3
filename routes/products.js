...
router.get('/:id', async (req, res) => {
  try {
    ...
    if (!product) return res.status(404).send({ success: false, message: "Sản phẩm không tồn tại" });
...
router.post('/', authorize(["mod"]), async function(req, res, next) {
  try {
...
router.put('/:id', authorize(["mod"]), async function(req, res, next) {
  try {
    let updateObj = {};
    let body = req.body;
    if(body.name){
